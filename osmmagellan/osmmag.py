        ...
        try:
            ...
            for osmfile in osmfiles:
                ...
                if name:
                    ...
                    m.name = "OpenStreetMap derived map generated by osmmag V%s"%version
...
def main():
    ...
    parser = OptionParser(usage='usage: %prog [options] [file1.osm] [file2.osm] [...]',
                          version=version,
                          prog='osmmag',
                          description='OpenStreetmap to Magellan map converter, by Henrik Johansson (henjo2006@gmail.com)')

    parser.add_option('-o', '--output', dest='output',
                      help='output Magellan GPS image file', metavar='FILE')

    parser.add_option('-n', '--name', dest='name', help='name of the generated map', metavar='NAME')

    parser.add_option('-r', '--rules', dest='rulefile', help='map generation rule file', metavar='FILE')

    parser.add_option('-b', '--bbox', dest='bbox', type='float', nargs=4, metavar='minLon minLat maxLon maxLat',
                      help='download OSM data within bounding box')

    parser.add_option('-q', '--quantization', dest='scale', type='float', nargs=2, metavar='lonstep, latstep',
                      help='quantization step used when the floating point coordinates are converted to integer. '
    ...
    parser.add_option('-t', '--topo', dest='topo', metavar='FILE',
                      help='add topographical information from FILE which is a '
                           'georeferenced raster file in WGS84 reference system that is supported by GDAL')

    parser.add_option('--topostartscale', dest='topostartscale', metavar='INDEX', type='int',
                      help='index to first scale level that will contain topo contours')

    parser.add_option('--topointervals', dest='topointervals', metavar='L0,L1,...', type='string',
                      action='callback', callback=parseIntList,
                      help='topo contour intervals for each scale level')

    parser.add_option('--name-tag-list', dest='nametaglist', metavar='nametag1,nametag2...', type='string',
                      action='callback', callback=parseStringList,
                      help='Specify the tag that will be used to supply the name.  Useful for language ' \
    ...
    parser.add_option('--big-endian', action='store_true', dest='bigendian', default=False,
